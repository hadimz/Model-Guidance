...
lambda_=[1e-3, 1e-4, 5e-4, 5e-3, 5e-5]
layer=["Final"]
localization_loss_fn=["Energy_Points"]
feedback_type=["points_adaptive"]
similarity_threshold=[0.99]#, 0.999, 97, 0.95, 0.997, 0.995]
adaptive_lambda=[False, True]
num_guiding_points=[25, 5, 1, 10, 50]
learning_rates=[1e-4]
adaptive_threshold = [0.25, 0.5, 0.75]
backbone =[
...
if __name__ == "__main__":
    ...
    output_file = f'logs/COCO_Final/points_adaptive/{backbone}/noprob_{backbone}_COCO_lr_{lr}_AttMethod_{attribution_method}_feedback_type_{ftype}_localization_loss_{locloss}_Lambda_{loss_lambda}_numGuidingPoints_{ngp}_AdaptiveLambda_{al}_SimilarityThreshold_{threshold}_adaptive_threshold_{at}_seed_0.out'
    error_file  = f'logs/COCO_Final/points_adaptive/{backbone}/errors/noprob_{backbone}_COCO_lr_{lr}_AttMethod_{attribution_method}_feedback_type_{ftype}_localization_loss_{locloss}_Lambda_{loss_lambda}_numGuidingPoints_{ngp}_AdaptiveLambda_{al}_SimilarityThreshold_{threshold}_adaptive_threshold_{at}_seed_0.err'
